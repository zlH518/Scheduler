...
if __name__ == "__main__":
    algorithm_names = ['FCFS', 'FCFS_With_Migrate']
    palette = sns.color_palette("Set2")
    line_styles = ['-', '--', ':']  # 实线、虚线、点线
    all_data = []
    for algorithm_name in algorithm_names:
    ...
    plt.figure(figsize=(10, 6))
    for index, algorithm_name in enumerate(algorithm_names):
        plt.plot(df['time'][index], df['average_queue_time'][index], linestyle=line_styles[0], color=palette[index],
                 label=algorithm_name)
    plt.title('Queue Time Over Time')
    plt.xlabel('Time')
    plt.ylabel('Average Queue Time of Completed Tasks')
    plt.legend()
    plt.show()
    ...
    plt.figure(figsize=(10, 6))
    for index, algorithm_name in enumerate(algorithm_names):
        plt.plot(df['time'][index], df['number_of_free_cards'][index], linestyle=line_styles[0], color=palette[index],
                 label=algorithm_name)
    plt.title('Free CardS Over Time')
    plt.xlabel('Time')
    plt.ylabel('Number of Free Cards')
    plt.legend()
    plt.show()
